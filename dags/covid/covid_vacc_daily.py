...
with DAG(
    ...
    def scrape_covid_vacc_daily(ds, **context):  
        '''
        Scrapes the daily raw data of Covid 19 Cases
        '''
        ...
        try:
            ...
            dist_df = dist_wide_df[~(dist_wide_df['state'] == 'TT')][['date','state','district','vaccinated1','vaccinated2']].copy().reset_index(drop=True)
            dist_df['total_doses_admn'] = dist_df['vaccinated1'] + dist_df['vaccinated2']
            dist_df.columns = ['date','state','district','first_dose_admn_district','second_dose_admn_district','total_doses_admn_district']
            dist_df = dist_df[dist_df['date'] == curr_date.strftime("%Y-%m-%d")].copy().reset_index(drop=True)
            dist_df['date'] = pd.to_datetime(dist_df['date'], format="%Y-%m-%d")

            merged_df = pd.merge(
                pd.merge(dist_df, state_df_grp, on=['date','state'], how='left'),
