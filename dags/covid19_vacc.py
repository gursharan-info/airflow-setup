...
def scrape_covid_vacc_daily(**context):
    ...
    curr_date = (datetime.fromtimestamp(context['execution_date'].timestamp()) - timedelta(day_lag) ).strftime("%Y-%m-%d")
    print(curr_date)
    ...
    delta_df = delta_df[delta_df['date'] == curr_date]
    # print(delta_df)
    ...
    filename = os.path.join(dir_path, 'covid19_vacc/covid_'+curr_date+'.csv')
    mapped_df.to_csv(filename,index=False)
    gupload.upload(filename, 'covid_'+curr_date+'.csv',gdrive_covid_vacc_folder)
...
dag = DAG("covid19VaccDailyScraping", default_args=default_args, schedule_interval="@daily")
